...
Jobs::Jobs(string location, string profession, string job_type, string jobUID, string experience, int max)
{
    ...
    Jobs temp;
    temp.set_job(location, profession, job_type, jobUID, experience, max_applications);
    all_jobs.push_back(temp);
}

void Jobs ::set_job(string location, string profession, string job_type, string jobUID, string experience, int max) {
    this-> location = location;
    this-> profession = profession;
    this-> job_type = job_type;
    this->jobUID = jobUID;
...
Jobs Jobs::create_job()
{
    ...
    cin >> experience;
    cout << "Do you want to limit the number of application?(1. yes, 2. no) :";
    cin >> app;
    if (app == 1) {
        cout << "Enter max number: ";
        cin >> max_applications;
    } else {
        max_applications = -1;
    }
    return Jobs(location,profession,job_type,jobUID,experience, max_applications);
}


void Jobs ::search_jobs() {
